...
import numpy as np
import math
from math import sin, cos, tan, atan, radians, atan2
...
def xyz2enu(center, v_xyz):
	...
	fm1 = 1.0/(1.0 - f)**2
	fai = atan2(fm1*center[2], rr)
	ram = atan2(center[1], center[0])
	rot = np.array(
		[[-sin(fai)*cos(ram), -sin(fai)*sin(ram), cos(fai)],
		 [-sin(ram)         ,  cos(ram)         , 0.0    ],
		 [ cos(fai)*cos(ram),  cos(fai)*sin(ram), sin(fai)]])
...
def llh2xyz(llh):
	...
	chi = sqrt((1.0 - e2*(math.sin(lat))**2))
	b = a*(1.0-e2)
	X = (a/chi + llh[2]) * cos(lat) * cos(lon)
	Y = (a/chi + llh[2]) * cos(lat) * sin(lon)
	Z = (b/chi + llh[2]) * sin(lat)
...
def calc(event):
	...
	try:
		alp = float(pydom["input#alp"][0].value)
		lat = float(pydom["input#lat"][0].value)
		lon = float(pydom["input#lon"][0].value)
		dep = float(pydom["input#dep"][0].value)
		str = float(pydom["input#str"][0].value)
		dip = float(pydom["input#dip"][0].value)
		rak = float(pydom["input#rak"][0].value)
		mag = float(pydom["input#mag"][0].value)
		chk = alp > 0.
